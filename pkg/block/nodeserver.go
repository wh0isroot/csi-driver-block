...
func (ns *NodeServer) NodePublishVolume(_ context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if blockFile == "" {
		return nil, status.Error(codes.InvalidArgument, fmt.Sprintf("%v is a required parameter", paramFsType))
	}
	if fstype == "" {
		return nil, status.Error(codes.InvalidArgument, fmt.Sprintf("%v is a required parameter", paramBlockFile))
	}
	source := blockFile
	...
	err = ns.mounter.Mount(source, targetPath, fstype, mountOptions)
	if err != nil {
