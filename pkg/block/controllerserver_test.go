...
const (
	testBlockFile                = "/tmp/test-block-file"
	testFsType                   = "ext4"
	testCSIVolume                = "volume-name"
	testVolumeID                 = "test-server/volume-name"
	newTestVolumeID              = "test-server##volume-name##"
	newTestVolumeWithVolumeID    = "test-server#volume-name#volume-name#"
	...
	}{
		{
			...
			req: &csi.CreateVolumeRequest{
				...
				Parameters: map[string]string{
					paramBlockFile:        testBlockFile,
					paramFsType:           testFsType,
			...
			resp: &csi.CreateVolumeResponse{
				Volume: &csi.Volume{
					...
					VolumeContext: map[string]string{
						paramFsType:           testBlockFile,
						paramBlockFile:        testCSIVolume,
						mountPermissionsField: "0750",
					},
				},
			},
...
func TestBlockVolFromId(t *testing.T) {
	cases := []struct {
		name      string
		volumeID  string
		resp      *blockVolume
		...
		{
			name:      "ID only server",
			volumeID:  testBlockFile,
			resp:      nil,
			expectErr: true,
		},
		{
			name:      "ID missing subDir",
			volumeID:  strings.Join([]string{testBlockFile}, "/"),
			resp:      nil,
			expectErr: true,
		},
		{
	...
	for _, test := range cases {
		test := test // pin
		t.Run(test.name, func(t *testing.T) {
			resp, err := getBlockVolFromID(test.volumeID)
...
func TestNewBLOCKVolume(t *testing.T) {
	cases := []struct {
		desc      string
		name      string
		size      int64
		params    map[string]string
		expectVol *blockVolume
		expectErr error
	}{
		{
			...
			params: map[string]string{
				paramFsType:    testBlockFile,
				paramBlockFile: "subdir",
			},
			expectVol: &blockVolume{
				id:        "block-server.default.svc.cluster.local#share#subdir#pv-name#",
				blockFile: testBlockFile,
				size:      100,
				uuid:      "pv-name",
				onDelete:  "delete",
		...
		},
		{
			...
			params: map[string]string{
				paramFsType:     testBlockFile,
				paramBlockFile:  fmt.Sprintf("subdir-%s-%s-%s", pvcNameMetadata, pvcNamespaceMetadata, pvNameMetadata),
				pvcNameKey:      "pvcname",
				pvcNamespaceKey: "pvcnamespace",
				pvNameKey:       "pvname",
			},
			expectVol: &blockVolume{
				id:        "block-server.default.svc.cluster.local#share#subdir-pvcname-pvcnamespace-pvname#pv-name#",
				blockFile: testBlockFile,
				size:      100,
				uuid:      "pv-name",
				onDelete:  "delete",
		...
		},
		{
			...
			expectVol: &blockVolume{
				id:        "block-server.default.svc.cluster.local#share#pv-name##",
				blockFile: testBlockFile,
				size:      200,
				uuid:      "",
				onDelete:  "delete",
		...
		},
		{
			...
			params:    map[string]string{},
			expectVol: nil,
			expectErr: fmt.Errorf("%s is a required parameter", paramFsType),
		},
		{
	...
	for _, test := range cases {
		vol, err := newBLOCKVolume(test.name, test.size, test.params, "delete")
		if !reflect.DeepEqual(err, test.expectErr) {
			t.Errorf("[test: %s] Unexpected error: %v, expected error: %v", test.desc, err, test.expectErr)
		}
...
func TestCopyVolume(t *testing.T) {
	cases := []struct {
				...
				VolumeContentSource: &csi.VolumeContentSource{
					Type: &csi.VolumeContentSource_Volume{
						Volume: &csi.VolumeContentSource_VolumeSource{
							VolumeId: "block-server.default.svc.cluster.local#share#subdir#src-pv-name",
						},
					},
				},
			},
			dstVol: &blockVolume{
				id:        "block-server.default.svc.cluster.local#share#subdir#dst-pv-name",
				blockFile: testBlockFile,
				uuid:      "dst-pv-name",
			},
			prepare: func() error { return os.MkdirAll("/tmp/src-pv-name/subdir", 0o777) },
		...
		},
		{
				...
				VolumeContentSource: &csi.VolumeContentSource{
					Type: &csi.VolumeContentSource_Snapshot{
						Snapshot: &csi.VolumeContentSource_SnapshotSource{
							SnapshotId: "block-server.default.svc.cluster.local#share#snapshot-name#snapshot-name#src-pv-name",
						},
					},
				},
			},
			...
			prepare: func() error {
				if err := os.MkdirAll("/tmp/snapshot-name/snapshot-name", 0o777); err != nil {
					return err
				}
...
func TestCreateSnapshot(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			req: &csi.CreateSnapshotRequest{
				SourceVolumeId: "block-server.default.svc.cluster.local#share#subdir#src-pv-name",
				Name:           "snapshot-name",
			},
			expResp: &csi.CreateSnapshotResponse{
	...
	cases := []struct {
	...
	}{
		{
			...
			prepare: func() error {
				...
				f, err := os.OpenFile("/tmp/snapshot-name/snapshot-name/src-pv-name.tar.gz", os.O_CREATE, 0o777)
				if err != nil {
					return err
				}
				return f.Close()
		...
		},
		{
			...
			req: &csi.DeleteSnapshotRequest{
				SnapshotId: "block-server.default.svc.cluster.local#share#snapshot-name#snapshot-name#src-pv-name",
				Secrets:    map[string]string{"mountoptions": "blockvers=4.1"},
			},
			expResp: &csi.DeleteSnapshotResponse{},
			prepare: func() error {
				if err := os.MkdirAll("/tmp/snapshot-name/snapshot-name/", 0o777); err != nil {
