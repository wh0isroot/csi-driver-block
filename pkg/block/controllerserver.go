...
type blockVolume struct {
	// Volume id
	id string
	// Address of the BLOCK server.
	// Matches paramServer.
	server string
	// Base directory of the BLOCK server to create volumes under
	// Matches paramShare.
	baseDir string
	// Subdirectory of the BLOCK server to create volumes under
	subDir string
	// size of volume
	size int64
...
func (cs *ControllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if err = cs.internalMount(ctx, blockVol, parameters, volCap); err != nil {
		return nil, status.Errorf(codes.Internal, "failed to mount block server: %v", err.Error())
	}
	defer func() {
	...
	setKeyValueInMap(parameters, paramSubDir, blockVol.subDir)
...
func (cs *ControllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	if !strings.EqualFold(blockVol.onDelete, retain) {
		...
		if err = cs.internalMount(ctx, blockVol, nil, volCap); err != nil {
			return nil, status.Errorf(codes.Internal, "failed to mount block server: %v", err.Error())
		}
		defer func() {
			if err = cs.internalUnmount(ctx, blockVol); err != nil {
		...
		if strings.EqualFold(blockVol.onDelete, archive) {
			archivedInternalVolumePath := filepath.Join(getInternalMountPath(cs.Driver.workingMountDir, blockVol), "archived-"+blockVol.subDir)
			if strings.Contains(blockVol.subDir, "/") {
				parentDir := filepath.Dir(archivedInternalVolumePath)
				klog.V(2).Infof("DeleteVolume: subdirectory(%s) contains '/', make sure the parent directory(%s) exists", blockVol.subDir, parentDir)
